...
class AutodialBatchReport implements FromCollection, ShouldQueue, WithHeadings, WithMapping
{
	use Exportable;
    public $data;
    ...
    */
    public function collection()
    {
    	...
    	$autocalls	= CommonSmsEmail::with('customer', 'contact', 'start_stage', 'end_stage', 'followup.GetQueryType', 'followup.GetQueryCategory', 'followup.GetSubQueryCategory', 'followup.GetCustomerNature', 'followup.GetPriority', 'followup.GetQueryStatus', 'followup.GetEscalateUser')
      					->where('ori_common_sms_email.batch_id', $batch->id)
          				->where('ori_common_sms_email.sent_type', config('constant.CH_AUTODIAL'))
          				->get();

		$export_data	= array();
		foreach ($autocalls as $call)
		{
			$call_data	= array();
