...
class ProcessCampaignSmsBatches implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            $batch = CampaignBatch::where('ori_campaign_batches.status', config('constant.INACTIVE'))
                                    ->where('ori_campaign_batches.channel_type', config('constant.CH_SMS'))
            ...
            do {
                ...
                $contacts = GroupContact::select('ori_group_contacts.id','ori_group_contacts.contact_id','ori_group_contacts.group_id','ori_cmp_contacts.mobile','ori_cmp_contacts.country_code','ori_cmp_contacts.user_id', 'ori_cmp_contacts.first_name', 'ori_cmp_contacts.last_name', 'ori_cmp_contacts.email')
                ...
                foreach ($contacts as $contact)
                {
                    ...
                    if(isset($survey_details->is_lang1) && $survey_details->is_lang1 == config('constant.LANG_ENG'))
                    {
                        ...
                        if($eng_q_count > 0)
                        {
                            ...
                            $survey_url.='For English Survey '.$e_url.' ';
                    ...
                    if(isset($survey_details->is_lang2) && $survey_details->is_lang2 == config('constant.LANG_MALA'))
                    {
                        ...
                        if($mal_q_count > 0)
                        {
                            ...
                            $survey_url.='For Malayalam Survey '.$m_url;
        ...
        catch(\Illuminate\Database\QueryException $ex)
        {
            ...
            $cron_logid           = $cc_cron_log->createLog('process_campaign_sms_batches');
            $error      = $ex->getMessage();
