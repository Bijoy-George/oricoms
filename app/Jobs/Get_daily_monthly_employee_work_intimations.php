...
class Get_daily_employee_work_intimations implements ShouldQueue
{
    ...
    public function handle()
    {
		try{
			...
			if ($intimation_settings && $intimation_settings->daily_view_deatils_intimations && !empty($intimation_settings->daily_view_deatils_intimations_mail))
			{
					$mail_template  = Templates::find($intimation_settings->daily_view_deatils_intimations_mail);
					if ($mail_template)
					{
							...
							$table = Helpers::escalate_projecttaskhour_mail_content($cmpny_id,$user_id,config('constant.INTIMATION_DAILY'));
