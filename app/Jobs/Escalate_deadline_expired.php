...
class Escalate_deadline_expired implements ShouldQueue
{
    ...
    public function handle()
    {
	   ...
	   $results = Helpdesk::where('escalation_due_date','like','%'.$dt.'%')->whereNotIn('query_status',$status_res)->where('cmpny_id',$cmpny_id)->where('escalation_status',1)->get();
	   $reporting = array();
	   $to_data = array();
	   $dockets = '';
	   if(count($results)>0)
	...
	//   $reporting = array_unique($reporting);//echo "<pre>";print_r($reporting);
	   if(count($to_data)>0)
	   {
		   foreach($to_data as $to)
		   {
			   ...
			   $email = $details->email;
			   $name = $details->name;
			   $docketsresults = Helpdesk::select('docket_number','id')->where('escalation_due_date','like','%'.$dt.'%')->whereNotIn('query_status',$status_res)->where('escalate',$to)->where('cmpny_id',$cmpny_id)->where('escalation_status',1)->get();
