...
jsxc = {
   ...
   getFormattedTime: function(unixtime) {
      ...
      if(hours>=12)
      {
        var hour = parseInt(hours) - parseInt(12);
        if (hour==0)
        {
          modtime = hours + ':' + minutes + ' PM';
        }
        else
        {
          modtime = hour + ':' + minutes + ' PM';
        }
      }
      else
      {
        modtime = hours + ':' + minutes + ' AM';
      }

      var date = (typeof msgDate.toLocaleDateString === 'function') ? msgDate.toLocaleDateString() : day + '.' + month + '.' + year;
      var time = (typeof msgDate.toLocaleTimeString === 'function') ? modtime : hours + ':' + minutes;
      // compare dates only
   ...
   init: function(options) {
      ...
      if(options && options.external_data_url && typeof external_data_url !== 'undefined'){
        jsxc.external_data_url = options.external_data_url;
      }
...
jsxc.xmpp = {
   ...
   onConnfail: function(ev, condition) {
     // console.log("ev : "+JSON.stringify(ev)+" and condition : "+condition);
   ...
   onPresence: function(presence) {
      ...
      if(jsxc.jidToBid(to) == jsxc.jidToBid(jsxc.xmpp.conn.jid)){
...
jsxc.gui = {
   ...
   update: function(bid) {
      ...
      var avl_status = $.t(jsxc.CONST.STATUS[data.status]);
      ue.find('.jsxc_name:first').add(spot).html(data.name).attr('title', bid + ' ' + $.t('is_', {
         status: $.t(jsxc.CONST.STATUS[data.status])
      }));
      if(avl_status=="offline")
      {
        ue.find('.user-offline-msg').show();
		ue.find('.avl_status_user').html("User has signed out/inactive/not online");
      }
      else {
        ue.find('.user-offline-msg').fadeOut();
		//ue.find('.avl_status_user').html("");
      }
      ue.find('.jsxc-upload:first').attr('bid', bid);
      var custom_bid=bid;
      custom_bid=custom_bid.replace("@localhost","");
      ue.find('.jsxc-upload:first').attr('id', 'file-upload-btn-'+custom_bid);
      ue.find('.jsxc-upload:first').attr('onclick', "openFileDialog('"+custom_bid+"')");
      ue.find('.jsxc-file-upload:first').attr('id', 'file-upload-'+custom_bid);
      ue.find('.jsxc-file-upload:first').attr('onchange', "uploadFile(event,'"+custom_bid+"')");
      ue.find('.file-upload-progress-div:first').attr('id','file-upload-progress-div-'+custom_bid);
      // ue.find('.file-upload-name:first').attr('id', 'file-upload-name-'+custom_bid);
      ue.find('.error-div:first').attr('id', 'ed-'+custom_bid);
      ue.find('.jsxc_textinput:first').attr('onpaste','xmInputMessageOnpaste('+custom_bid+')');
      ue.find('.jsxc_textinput:first').attr('id', 'jt-'+custom_bid);
      ue.find('.auto_reply:first').attr('id','ar-'+custom_bid);
   ...
   updatePresence: function(bid, pres) {
      ...
      $('[data-bid="' + bid + '"]').each(function() {
         ...
         el.attr('data-status', pres);
         // console.log("bid : "+bid+" and pres: "+pres);
   ...
   shortnameToImage: function(str) {
      str = str.replace(jsxc.gui.regShortNames, function(shortname) {
         ...
         if (jsxc.gui.emoticonList.core[shortname]) {
            ...
            // src = jsxc.options.root + '/../../jsxc/build/lib/emojione/assets/svg/' + filename + '.svg';
            src = 'jsxc/build/lib/emojione/assets/svg/' + filename + '.svg';
...
jsxc.gui.roster = {
    ...
    */
   add: function(bid) {
      var booleann = jsxc.gui.roster.checkNickNameSet(bid);
      if(!booleann){
        if(jsxc.options.external_data_url && typeof jsxc.options.external_data_url !== 'undefined'){
            var formData = new FormData();
            var usr_name = jsxc.gui.roster.getName(bid);
            formData.append('reg_id',usr_name);
            var xxhr = httpPost(jsxc.options.external_data_url,formData);


            xxhr.onreadystatechange = function() {//Call a function when the state changes.
                if(xxhr.readyState == 4 && xxhr.status == 200)
                {
                    ...
                    if(response){
                      ...
                      if(response.status == 1 && response.name){
                        /* var is_ksfe_customer = response.ksfe_id_flag;
                        var image=""; */
                        /* if(is_ksfe_customer)
                        {
                          image = "<img src='"+general_url+"/images/avatar-20.png' alt='img'>"
                        }
                        else
                        {
                          image = "<img src='"+general_url+"/images/avatar-20-cross.png' alt='img-cross'>"
                        } */
                        //jsxc.gui.roster._rename(bid,image+response.name);
                        jsxc.gui.roster._rename(bid,response.name);
   ...
   checkNickNameSet:function(user_id){
   ...
   noConnection: function() {
      ...
      $('<p>' + $.t('no_connection') + '</p>').append(' <a>' + $.t('restart') + '</a>').click(function() {
         jsxc.storage.removeUserItem('forcedLogout');
   ...
   empty: function() {
      ...
      var text = $('<p> Your chat roster has been cleared or is vacant...</p>');
...
jsxc.gui.window = {
   ...
   init: function(bid) {
      ...
      win.find('.jsxc_bar').click(function(e)
      {
        if(e.target)
        {
          ...
          {
            if($(e.target).attr('class') == "jsxc_name")
            {
              // var bbid=bid.split("@");
              // var custid=bbid[0];
              // window.location.replace(general_url+"/profile/0/0/0/0/"+custid);
              // window.open(general_url+"/profile/0/0/0/0/"+custid);
      ...
      win.find('.jsxc_profile').click(function() {
         var bbid=bid.split("@");
         var custid=bbid[0];
         window.open(general_url+"/profile/0/"+custid,"_blank");
      });
      ...
      win.find('.jsxc_textinput').keyup(function(ev) {
         var body = $(this).val();
         var bidid = bid;
         var bididsplit = bidid.split("@");
         if(ev.which==72)
          {
            fetchAutoReplyCategory(0,bididsplit[0]);
            $("#ar-"+bididsplit[0]).css("display","block");
         }
         else
         {
            $("#ar-"+bididsplit[0]).css("display","none");
         }
         ...
         else if(body.length>280)
         {
           var bidid = bid;
           var bididsplit = bidid.split("@");
           console.log(bididsplit[0]);
           $("#ed-"+bididsplit[0]).css("display","block");
           $('#ed-'+bididsplit[0]+' .warning_message').html("Content has reached beyond the limit. Maximum 280 characters allowed.");
           setTimeout(function(){
             //$("#jt-"+bididsplit[0]).val('');
             $("#ed-"+bididsplit[0]).css("display","none");
           },2000);
           return;
         }
         else
   ...
   _postMessage: function(message, restore) {
      ...
      if(msg_split[0]=="Sending")
      {
        ...
        if(direction=="in")
        {
          var ab = "User has sent the file "+general_url+"/uploads/chat_documents/"+doc_savedname;
        }
        else if(direction=="out")
        {
          var ab = "Agent has sent the file "+general_url+"/uploads/chat_documents/"+doc_savedname;
        }
        msg = ab;
      }
...
jsxc.notification = {
   ...
   playSound: function(soundFile, loop, force) {
      ...
      var audio = new Audio(general_url+'/jsxc/build/sound/' + soundFile);
      audio.load();
      ...
      if (jsxc.storage.getUserItem('key') === null) {
         ...
         if (Worker) {
            ...
            try {
               ...
               worker = new Worker(root + 'jsxc/build/lib/otr/lib/dsa-webworker.js');
         ...
         if (!jsxc.otr.dsaFallback) {
            ...
            // start worker
            worker.postMessage({
               imports: [root + 'jsxc/build/lib/otr/vendor/salsa20.js', root + 'jsxc/build/lib/otr/vendor/bigint.js', root + 'jsxc/build/lib/otr/vendor/crypto.js', root + 'jsxc/build/lib/otr/vendor/eventemitter.js', root + 'jsxc/build/lib/otr/lib/const.js', root + 'jsxc/build/lib/otr/lib/helpers.js', root + 'jsxc/build/lib/otr/lib/dsa.js'],
               seed: BigInt.getSeed(),
               debug: true
            });
...
'            </div>\n' +
'            <div class="jsxc_profile">P</div>\n' +
'            <div class="jsxc_close" style="float:right">×</div>\n' +
'         </div>\n' +
...
// '                           </div>\n' +
// '                       </div>\n' +
'                       <div class="user-offline-msg">\n'+
'                           <span class="avl_status_user"></span>\n'+
'                       </div>\n'+
'                       <div class="reply-suggestion">\n'+
'                           <ol class="auto_reply"></ol>\n'+
'                       </div>\n'+
...
'                           </div>\n'+
'                       </div>\n'+
'                       <div class="file-upload-progress-div" style="display:none;">'+
'                         <div class="clearfix file-upload-progress-subdiv">'+
'                           <span class="upload_label"><b><i class="fa fa-spinner fa-spin" style="font-size:18px"></i> Uploading </b></span><span class="file-upload-name"></span>'+
'                           <br><span class="file-upload-message"></span>'+
'                         </div>'+
'                       </div>'+
'                       <textarea class="jsxc_textinput" data-i18n="[placeholder]Message"></textarea>\n' +
'                       <button id="" class="btn jsxc-upload" title="Upload" onclick=""><i class="far fa-1.5x fa-folder"></i></button>\n'+
'                       <input id="" class="jsxc-file-upload" type="file" onchange=""/>\n'+
...
function openFileDialog(custombid)
{
  $('#file-upload-'+custombid).click();
}

function uploadFile(event,cbid)
{
    var uploadflag=1;
    var errorMessage = "";
    $("#file-upload-progress-div-"+cbid).show();
    ...
    if (files.length < 1)
    {
      ...
      setTimeout(function(){
        $("#file-upload-progress-div-"+cbid).hide();
      },5000);
      return;
    }
    var file = files[0];
          ...
          success: function(data)
          {
            ...
            if(jQuery.isEmptyObject(errdata))
            {
              ...
              var message=
              {
                ...
                encrypted:false,
                forwarded:false,
                htmlMsg:documentName,
                msg:documentName,
                stamp:time.getTime(),
                type:"plain",
                _received:false,
                _uid:time.getTime()+":msg"
              };
              jsxc.gui.window.postMessage(message);
              $("#file-upload-progress-div-"+cbid).hide();
            }
...
function xmInputMessageOnpaste(bid)
{
  var messageContent="";
  setTimeout(function ()
  {
    messageContent = $("#jt-"+bid).val();
    console.log("messageContent: "+messageContent);
    if(messageContent.length >280)
    {
      $("#ed-"+bid).css("display","block");
      $('#ed-'+bid+' .warning_message').html("Content has reached beyond the limit. Maximum 280 characters allowed.");
      setTimeout(function(){
        //$("#jt-"+bid).val('');
        $("#ed-"+bid).css("display","none");
      },2000);
    }
    else
    {
...
function fetchAutoReplyCategory(parent_cat,bidid)
{
  var cmpny_id = sessionStorage.getItem('cmpny_id');
  $.ajax({
          url: '/api/getAutoReplyCategory',
          type: 'POST',
          data: "cmpny_id="+cmpny_id+"&parent_cat="+parent_cat,
          dataType: 'json',
          success: function(data)
          {
            $("#ar-"+bidid).empty();
            var auto_reply_cat_length = data.auto_reply_cat.length;
            if(auto_reply_cat_length>0)
            {
              for(i=0;i<auto_reply_cat_length;i++)
              {
                $("#ar-"+bidid).append('<li id="ar'+data.auto_reply_cat[i].id+'" onclick="auto_reply_cat_clicked('+data.auto_reply_cat[i].id+','+bidid+')">'+data.auto_reply_cat[i].name+'</li>');
              }
            }
            var faq_cat_length = data.faq_cat.length;
            if(faq_cat_length>0)
            {
              //$("#ar-"+bidid).empty();
              for(j=0;j<faq_cat_length;j++)
              {
                $("#ar-"+bidid).append('<li id="fa'+data.faq_cat[j].id+'" tabindex="0" onclick="faq_cat_clicked('+data.faq_cat[j].id+','+bidid+')">'+data.faq_cat[j].category_name+'</li>');
              }
            }
...
function auto_reply_cat_clicked(categoryid,bidid)
{
  fetchAutoReplies(categoryid,bidid);
}

function faq_cat_clicked(categoryid,bidid)
{
  fetchFaqReplies(categoryid,bidid);
}

function fetchAutoReplies(categoryid,bidid)
{
  var cmpny_id = sessionStorage.getItem('cmpny_id');
  $.ajax({
          url: '/api/getAutoReplies',
          type: 'POST',
          data: "cmpny_id="+cmpny_id+"&cat_id="+categoryid,
          dataType: 'json',
          success: function(data)
          {
            var auto_reply_length = data.auto_reply.length;
            if(auto_reply_length>0)
            {
              $("#ar-"+bidid).empty();
              for(i=0;i<auto_reply_length;i++)
              {
                var reply = data.auto_reply[i].reply;
                $("#ar-"+bidid).append('<li id="'+data.auto_reply[i].id+'" tabindex="'+i+'" reply="'+data.auto_reply[i].reply+'" onclick="add_auto_reply_to_textbox('+data.auto_reply[i].id+','+bidid+')">'+data.auto_reply[i].reply+'</li>');
              } 
            }            
          },
          error: function(jqXHR, textStatus, errorThrown)
...
function fetchFaqReplies(categoryid,bidid)
{
  var cmpny_id = sessionStorage.getItem('cmpny_id');
  $.ajax({
          url: '/api/getFaqReplies',
          type: 'POST',
          data: "cmpny_id="+cmpny_id+"&cat_id="+categoryid,
          dataType: 'json',
          success: function(data)
          {
            var faq_length = data.faq.length;
            if(faq_length>0)
            {
              $("#ar-"+bidid).empty();
              for(i=0;i<faq_length;i++)
              {
                var questionLang1 = data.faq[i].query_title_lang1;
                var questionLang2 = data.faq[i].query_title_lang2;
                if(questionLang1 === null)
                {
                  question = questionLang2;
                }
                else
                {
                  question = questionLang1;
                }

                var answerLang1 = data.faq[i].answer_lang1_short;
                var answerLang2 = data.faq[i].answer_lang2_short;
                if(answerLang1 === null)
                {
                  answer = answerLang2;
                }
                ...
                $("#ar-"+bidid).append('<li id="'+data.faq[i].id+'" tabindex="'+i+'" answer="'+answer+'" onclick="fetch_auto_answer_to_textbox('+data.faq[i].id+','+bidid+')">'+question+'</li>');
              } 
            }            
          },
          error: function(jqXHR, textStatus, errorThrown)
...
function add_auto_reply_to_textbox(auto_reply_id,bidid)
{
  var reply = $("#ar-"+bidid+" #"+auto_reply_id).attr('reply');
  $("#jt-"+bidid).val('');
  $("#jt-"+bidid).val(reply);
}

function fetch_auto_answer_to_textbox(faq_id,bidid)
{
  var answer = $("#ar-"+bidid+" #"+faq_id).attr('answer');
  $("#jt-"+bidid).val('');
  $("#jt-"+bidid).val(answer);
}