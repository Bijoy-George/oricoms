...
var xmppConnection = new function(){

   var this_obj = this;

   var default_settings = {
      xmpp_connction_url : undefined,
      host:undefined,
      resource:undefined,
      user_name:undefined,
      password:undefined,
   ...
   this.initConnection = function(settings){
      this_obj.configureSettings(settings);
   },
   this.configureSettings = function(settings){
      if(!settings.xmpp_connction_url || typeof settings.xmpp_connction_url === 'undefined'){
         return
      }
      if(!settings.host || typeof settings.host === 'undefined'){
         return
      }
      if(!settings.resource || typeof settings.resource === 'undefined'){
         return
      }
      if(!settings.user_name || typeof settings.user_name === 'undefined'){
         return
      }
      if(!settings.password || typeof settings.password === 'undefined'){
         return
      }
       if(!settings.external_data_url || typeof settings.external_data_url === 'undefined'){
         return
      }

      default_settings.xmpp_connction_url = settings.xmpp_connction_url;
      default_settings.host = settings.host;
      default_settings.resource = settings.resource;
      default_settings.user_name = settings.user_name;
      default_settings.password = settings.password;
      default_settings.external_data_url = settings.external_data_url;

      this_obj.startView(default_settings);

   },
   this.startView = function(default_settings){
      

            var settings = {
               xmpp: {
                  url: default_settings.xmpp_connction_url,
                  domain: default_settings.host,
                  resource: default_settings.resource,
                  overwrite: true
               }
            };
            ...
            jsxc.init({
              ...
              /* numberOfMsg: 100,*/
               ...
               external_data_url : default_settings.external_data_url
            });

            ///////////////////////*******************************************//////////////////////////

            var user_name = default_settings.user_name;          // this is the username of the chat login   
            var password = default_settings.password;          // this is the password for the user login

            

            jsxc.xmpp.login(user_name +'@'+settings.xmpp.domain , password);   /// Log in the chat 
            ...
            //jsxc.start( 'rateesh123'+'@'+ settings.xmpp.domain, 'rateesh123');
            ...
            $('#bosh-url, #xmpp-domain').on('input', function() {
               ...
               timeout = setTimeout(function() {
                  this_obj.testBoshServer($('#bosh-url').val(), $('#xmpp-domain').val(), function(result) {
          ...
          */
   this.testBoshServer = function(url, domain, cb) {
