...
@section('title')
{{config('constant.site_title')}} - {{Auth::user()->getCompany->ori_cmp_org_name}} - Feedbacks
@endsection
@section('content')

 <form action="{{url('/search_feedbacklist')}}" method="POST" class="listing form-common" name="form-common">
<aside class="sidebar">
...
<div class="content-area">
    ...
    <div class="col-sm-5">
      <h2 class="m-0">{{__('Feedback')}} </h2>
      <small>Feedback settings</small>
    </div>
    <div class="col-sm-7 text-right"><a href="{{url('settings')}}" class="btn btn-danger"><img src="{{ asset('img/ic_arrow_back.svg') }}" alt=""></a>
	   <a href="{{route('feedback.create')}}" class="btn btn-success ml-2"><img src="{{ asset('img/ic_add.svg') }}"  alt=""/> Add New Feedback Settings</a>
