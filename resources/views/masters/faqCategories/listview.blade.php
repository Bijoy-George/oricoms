        ...
        <thead>
                ...
                <tbody>
					...
					<tr>
						...
						<td><strong>{{$faqCategory->category_name}}</strong></td>
						<td><strong>{{$faqCategory->short_code}}</strong></td>
						<td><strong>{{$faqCategory->slug}}</strong></td>
						<td>
						@if($faqCategory->parent_category_id != NULL) 					{{$faqCategory->GetParent->category_name}} 
						@endif</td>
						...
						<a href="{{route('faq_categories.edit', $faqCategory->id)}}"  class="btn btn-outline-secondary btn-sm"><i class="far fa-edit"></i></a></td>
