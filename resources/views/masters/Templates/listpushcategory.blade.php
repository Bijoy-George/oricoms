...
<ul class="email-list" id="sms_lists1">
  ...
  <?php 
        ...
        ?>
  @if($k==1)
  <input type="hidden" id="current_temp_id_push" value="{{$id}}"/>
  @endif
  <li id="default pushlist-id-{{$id}}" class="default" onclick="javascript:select_push_template('{{$id}}');">
  	<h2 class="email-id"><i class="far fa-comment"></i> {{$thread->title}}</h2>
    <span class="push-subject">{{$thread->subject}}</span></li>
  @endforeach
</ul>
