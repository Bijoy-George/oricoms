...
@endsection
@section('content')
 <form action="{{url('/search_supply_card')}}" method="POST" class="listing form-common" name="form-common">
<aside class="sidebar">
...
<div class="content-area">
    ...
    <div class="col-sm-5">
      <h2 class="m-0">{{__('Supply Cards')}} <span id="totalcount"></span></h2>
      <small>Available Cards</small>
    </div>
    <div class="col-sm-7 text-right"><a href="{{url('settings')}}" class="btn btn-danger"><img src="{{ asset('img/ic_arrow_back.svg') }}" alt=""></a>
      @if( Helpers::checkPermission('supply card create'))
	   <a href="{{route('supply_card.create')}}" class="btn btn-success ml-2"><img src="{{ asset('img/ic_add.svg') }}"  alt=""/> Add New Card</a>
		    @endif
    </div>
  </header>
