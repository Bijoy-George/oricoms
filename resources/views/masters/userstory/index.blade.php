...
@endsection
@section('content')
<form action="{{url('/search_userstory')}}" method="POST" class="listing form-common" name="form-common">
  <aside class="sidebar">
  <div class="search-box">
    ...
    <div class="content-area">
      <header class="row align-items-center">
        <div class="col-sm-5">
          <h2 class="m-0">{{__('User Story')}} <span id="totalcount"></span></h2>
          <small>User Story</small> </div>
          <input type="hidden" name="project_id" id="project_id" value="{{$project_id}}">
		  <input type="hidden" name="sprint_id" id="sprint_id" value="{{$sprint_id}}">
		<div class="col-sm-7 text-right"><a href="{{url('scrumboard')}}/{{$project_id}}}" class="btn btn-primary ml-2"><img src=""  alt=""/> Scrum Board </a><a href="{{url('sprint')}}/{{$project_id}}}" class="btn btn-danger"><img src="{{ asset('img/ic_arrow_back.svg') }}" alt=""></a>
      </header>
