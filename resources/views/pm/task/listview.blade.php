<input type="hidden" name="list_count" id="list_count" value="@isset($list_count){{$list_count}}@endisset">
      ...
      <table width="100%" id="querytype_lists" class="table">
        <thead>
                ...
                <tbody>
					...
					@php $pstatus = $pjct_dets->status ?? NULL @endphp
					@php $pjct_due_date = $pjct_dets->due_date ?? NULL @endphp
					...
					@if( Helpers::checkPermission('task delete'))
					<a href="javascript:void(0)" onclick="deletePop('project_task_pm/' + {{ $res->id }},{{ $res->id }})" class="btn btn-outline-danger">
    ...
    <!-- 
     ...
     -->
	 {{-- <div class="table-widget col-md-4" style="display:none;">
      <table width="100%" id="querytype_lists" class="table">
        <thead>
                <tr>
					<th width="30" class="text-center">{{__('#')}}</th>					
					<th>{{__('Projects')}}</th>
					<th>{{__('No of Tasks')}}</th>
					<th>{{__('Hours Allocated')}}</th>
					<th>{{__('Spend Hours')}}</th>
				</tr>
                </thead>
                <tbody>
