...
@section('title')
{{config('constant.site_title')}} - {{Auth::user()->getCompany->ori_cmp_org_name}} - Auto Replies
@endsection
@section('content')

 <form action="{{url('/search_autoreplylist')}}" method="POST" class="listing form-common" name="form-common">
<aside class="sidebar">
...
<div class="content-area">
    ...
    <div class="col-sm-5">
      <h2 class="m-0">{{__('Auto Replies')}} <span id="totalcount"></span></h2>
      <small>Available Auto Replies</small>
    </div>
    <div class="col-sm-7 text-right"><a href="{{url('settings')}}" class="btn btn-danger"><img src="{{ asset('img/ic_arrow_back.svg') }}" alt=""></a>
	   <a href="{{route('chat_auto_reply.create')}}" class="btn btn-success ml-2"><img src="{{ asset('img/ic_add.svg') }}"  alt=""/> Add New Auto Reply</a>
	</div>
  </header>
